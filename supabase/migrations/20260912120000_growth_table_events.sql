...
begin
  ...
  elsif p_name = 'subscriptions_current' then
    return (select coalesce(jsonb_agg(to_jsonb(t)), '[]'::jsonb) from analytics.subscriptions_current t);
  ...
  elsif p_name = 'events' then
    return (select coalesce(jsonb_agg(to_jsonb(t)), '[]'::jsonb) from analytics.events t);
  else
    return '[]'::jsonb;
  end if;
end;
