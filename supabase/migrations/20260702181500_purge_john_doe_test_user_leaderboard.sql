-- Remove placeholder test accounts and exclude them from the global leaderboard.

delete from auth.users u
using public.profiles p
where p.id = u.id
  and lower(trim(coalesce(p.display_name, ''))) in ('john doe', 'test user');
...
as $$
  with scored as (
    ...
    where coalesce(u.raw_app_meta_data->>'leaderboard_seed', 'false') <> 'true'
      ...
      and lower(trim(coalesce(p.display_name, ''))) not in ('john doe', 'test user')
      and (
        p.id = auth.uid()
