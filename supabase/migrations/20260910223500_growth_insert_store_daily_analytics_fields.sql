drop function if exists public.growth_insert_store_daily(text, date, text, text, int, int);
...
begin
  insert into analytics.store_daily_metrics (
    ...
    first_time_downloads, acquisitions, impressions, product_page_views, redownloads
  ) values (
    p_platform, p_metric_date, p_country, p_source,
    p_first_time_downloads, p_acquisitions, p_impressions, p_product_page_views, p_redownloads
  )
...
revoke all on function public.growth_insert_store_daily(text, date, text, text, int, int, int, int, int) from public, anon, authenticated;
grant execute on function public.growth_insert_store_daily(text, date, text, text, int, int, int, int, int) to service_role;
