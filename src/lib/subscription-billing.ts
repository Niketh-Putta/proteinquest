...
function productIdToPlanLabel(productId: string): string {
  if (
    ...
    /year|annual/i.test(productId)
  ) {
    return 'Yearly';
  }
  if (
    productId === REVENUECAT_PRODUCT_IDS.weekly ||
    /week/i.test(productId)
  ) {
    return 'Weekly';
  }
  return 'Pro';
...
function buildDetails(info: CustomerInfo): SubscriptionBillingDetails {
  ...
  const memberSince =
    parseIsoDate(entitlement?.originalPurchaseDate ?? subscription?.originalPurchaseDate ?? info.originalPurchaseDate) ??
    purchaseHistory.at(-1)?.purchaseDate ??
    null;
...
/** One-line summary for Settings billing row. */
export function subscriptionSummaryLine(details: SubscriptionBillingDetails): string {
  if (details.status === 'cancelled' && details.accessUntil) {
    return `Pro until ${formatBillingDate(details.accessUntil)} · cancelled`;
  }
  if (details.status === 'billing_issue') {
    return 'Payment issue — update in store';
  }
  if (details.isPro && details.nextBillingDate) {
    return `${details.planLabel} · renews ${formatBillingDate(details.nextBillingDate)}`;
  }
  if (details.isPro) {
    return `${details.planLabel} · active`;
  }
  if (details.status === 'expired') {
    return 'Pro expired · resubscribe anytime';
  }
  return 'Payment method, restore, and plan options';
}
