...
export function nutritionEstimate(p: {
  ...
  const maintenance =
    (10 * p.weightKg +
      6.25 * p.heightCm -
      5 * p.age +
      (p.sex === 'female' ? -161 : p.sex === 'male' ? 5 : -78)) *
    (factors[p.activity] || 1.375);
  const reached =
  ...
  const adjustment = reached
    ...
    : p.goal === 'lose_fat'
      ...
      : p.goal === 'build_muscle'
        ...
        : 0;
  const calories =
    Math.round(Math.max(p.sex === 'female' ? 1200 : 1500, maintenance + adjustment) / 10) * 10;
  const multiplier = p.activity === 'sedentary' ? 1.2 : p.goal === 'lose_fat' ? 2 : 1.6;
  return {
    calories,
    protein: Math.round(Math.min(250, Math.max(45, p.weightKg * multiplier))),
  };
}
